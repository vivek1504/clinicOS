...
| `/patients/[id]/consultation` | Three columns: patient context with a link to the full record, **Doctor notes**, **Draft**. **Record** (shown only when the server has AssemblyAI configured) dictates into the notes: finished sentences are appended, the one still being recognised shows under the textarea, and the panel carries a red ring while the mic is open. **Structure notes** → honest processing state (label and elapsed time) → editable draft. AI-drafted rows sit on an iris tint; editing one flips it to doctor styling ("Doctor edited"). **Structure again** keeps the previous draft one click away. Saving with an empty structured note asks first. The page takes the room on open (a 409 shows who holds it) and releases it on leave. Redirects to the record if the appointment is already completed. |
